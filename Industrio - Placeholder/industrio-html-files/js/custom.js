...
/*
Created by: Parthiv Gandhi
Contact: +91 9998140005
*/
    ...
    if ($('.contact-form').length) {
        $('.contact-form').validate({ // initialize the plugin
            ...
            submitHandler: function(form) {
                ...
                var $status = $form.siblings('.form-status');

                if (!$status.length) {
                    $status = $('<p class="form-status" role="status"></p>');
                    $form.after($status);
                }
                ...
                fetch(form.action, {
                    ...
                    if (response.ok) {
                        $status.addClass('success').text('Thank you. Our team will contact you shortly.');
                        form.reset();
                        return;
                    }
                    throw new Error('Form submission failed');
                }).catch(function() {
                    $status.addClass('error').text('Something went wrong. Please use WhatsApp or try again.');
                }).finally(function() {
